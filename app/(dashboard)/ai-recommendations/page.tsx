...
const fallbackPriorityStyle = { badge: "bg-slate-100 text-slate-600", dot: "bg-slate-400" };
...
export default function AIRecommendationsPage() {
  return (
    <div className="p-6 space-y-6">
      ...
      {/* AI banner — removed conflicting border-0, using explicit border */}
      <Card className="shadow-sm bg-gradient-to-r from-violet-50 to-indigo-50 border border-violet-100">
      ...
      <div className="space-y-3">
        {recommendations.map((rec) => {
          const ps = priorityStyle[rec.priority] ?? fallbackPriorityStyle;
          return (
            <Card key={rec.id} className="border-0 shadow-sm hover:shadow-md transition-shadow">
              <CardContent className="p-4">
                <div className="flex items-start gap-4">
                  <div className={`h-2.5 w-2.5 rounded-full mt-1.5 flex-shrink-0 ${ps.dot}`} aria-hidden="true" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 flex-wrap">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${ps.badge}`}>{rec.priority}</span>
                      ...
                      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${statusStyle[rec.status] ?? "bg-slate-100 text-slate-500"}`}>{rec.status}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-4 mt-3 flex-wrap">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className={`text-[11px] font-semibold ${effortStyle[rec.effort] ?? "text-slate-600"}`}>{rec.effort}</span>
                      </div>
                      <div className="flex items-center gap-1">
                  ...
                  <ChevronRight className="h-4 w-4 text-slate-300 flex-shrink-0 mt-1" aria-hidden="true" />
                </div>
              </CardContent>
            </Card>
          );
