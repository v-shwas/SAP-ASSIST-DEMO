    ...
    <div className="p-6 space-y-6">
        ...
        <CardContent className="pt-0">
          <div className="flex gap-2">
            <div className="flex-1 relative">
              ...
              <label htmlFor="rag-search" className="sr-only">Search policy documents</label>
              <input
                id="rag-search"
          ...
          {searched && (
            <div className="mt-4 space-y-3">
              {ragResults.map((r) => (
                <div key={r.question} className="bg-white rounded-lg border border-blue-100 p-3">
                  ...
                  <div className="flex items-center gap-1 mt-2">
                    <File className="h-3 w-3 text-slate-400" aria-hidden="true" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <Card className="border-0 shadow-sm lg:col-span-1">
          ...
          <CardContent className="pt-0 space-y-0.5" role="group" aria-label="Document categories">
            {categoryList.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-2 py-1.5 rounded-md text-xs transition-colors ${isActive ? "bg-blue-50 text-blue-700 font-semibold" : "text-slate-600 hover:bg-slate-50"}`}
                >
                  <span className="flex items-center gap-2">
                    <FolderOpen className="h-3.5 w-3.5 opacity-60" aria-hidden="true" />
                    {cat.label}
                  </span>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          >
            <Upload className="h-6 w-6 mx-auto text-slate-400 mb-2" aria-hidden="true" />
                  ...
                  <div key={doc.name} className="flex items-center gap-3 py-2.5 border-b border-slate-50 last:border-0 hover:bg-slate-50 rounded px-2 -mx-2 transition-colors cursor-pointer group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <Tag className="h-2.5 w-2.5 text-slate-400" aria-hidden="true" />
                        <span className="text-[10px] text-slate-400">{doc.category}</span>
                        <Clock className="h-2.5 w-2.5 text-slate-400" aria-hidden="true" />
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${statusColor[doc.status] ?? "bg-slate-100 text-slate-600"}`}>{doc.status}</span>
                      <ChevronRight className="h-3 w-3 text-slate-300 group-hover:text-slate-500" aria-hidden="true" />
                    </div>
                  </div>
                ))}
              </div>
