...
import { Leaf, TrendingUp, TrendingDown, CheckCircle, ArrowRightLeft, Clock, Plus } from "lucide-react";
...
const txnBg: Record<string, string> = {
  Sold: "bg-red-50",
  Bought: "bg-blue-50",
  Earned: "bg-emerald-50",
};
...
function txnLabel(type: string, counterparty: string): string {
  if (type === "Bought") return `Bought from ${counterparty}`;
  if (type === "Sold") return `Sold to ${counterparty}`;
  return `Earned via ${counterparty}`;
}

function TxnIcon({ type }: { type: string }) {
  const cls = `h-3.5 w-3.5 ${txnStyle[type] ?? "text-slate-500"}`;
  if (type === "Sold") return <TrendingDown className={cls} aria-hidden="true" />;
  if (type === "Earned") return <CheckCircle className={cls} aria-hidden="true" />;
...
export default function CarbonCreditsPage() {
  ...
  function handleListCredits() {
    // Demo: close the form after submission
    setOfferQty("");
    setOfferPrice("");
    setShowOffer(false);
  }
    ...
    <div className="p-6 space-y-6">
          ...
          <Card key={s.label} className="border-0 shadow-sm">
            <CardContent className="p-4">
              <div className={`inline-flex h-7 w-7 items-center justify-center rounded-full ${s.bg} mb-3`}>
                <Leaf className={`h-3.5 w-3.5 ${s.color}`} aria-hidden="true" />
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-semibold text-slate-700 flex items-center gap-2">
                <ArrowRightLeft className="h-4 w-4 text-green-500" aria-hidden="true" /> P2P Listings
              </CardTitle>
              <Button size="sm" onClick={() => setShowOffer(!showOffer)} className="text-xs bg-emerald-600 hover:bg-emerald-700 text-white">
                <Plus className="h-3 w-3 mr-1" aria-hidden="true" /> Post Offer
              </Button>
            </div>
            ...
            {showOffer && (
              <div className="mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <div>
                    <label htmlFor="offer-qty" className="sr-only">Credits quantity</label>
                    <input
                      id="offer-qty"
                      className="w-full text-xs border border-emerald-200 rounded px-2 py-1 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-300"
                  ...
                  <div>
                    <label htmlFor="offer-price" className="sr-only">Price per credit in euros</label>
                    <input
                      id="offer-price"
                      className="w-full text-xs border border-emerald-200 rounded px-2 py-1 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-300"
                ...
                <div className="flex gap-2">
                  <Button size="sm" className="text-xs bg-emerald-600 hover:bg-emerald-700 text-white" onClick={handleListCredits}>
                    List Credits
                  </Button>
                ...
                <div key={l.id} className="flex items-center gap-3 p-3 border border-slate-100 rounded-lg hover:border-emerald-200 hover:bg-emerald-50/30 transition-colors cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {l.verified
                        ? <CheckCircle className="h-3 w-3 text-emerald-500" aria-label="Verified seller" />
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold text-slate-700 flex items-center gap-2">
              <Clock className="h-4 w-4 text-blue-500" aria-hidden="true" /> Transaction History
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="space-y-2">
              {transactions.map((tx) => (
                <div key={tx.id} className="flex items-center gap-3 py-2.5 border-b border-slate-50 last:border-0">
                  <div className={`h-7 w-7 rounded-full flex items-center justify-center flex-shrink-0 ${txnBg[tx.type] ?? "bg-slate-50"}`}>
                    <TxnIcon type={tx.type} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-slate-700">{txnLabel(tx.type, tx.counterparty)}</p>
                    <p className="text-[10px] text-slate-400">{tx.id} · {tx.date}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className={`text-xs font-semibold ${txnStyle[tx.type] ?? "text-slate-600"}`}>{tx.type === "Sold" ? "-" : "+"}{tx.credits} cr.</p>
                    {tx.value > 0 && <p className="text-[10px] text-slate-400">€{tx.value.toLocaleString()}</p>}
                  </div>
                  <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded flex-shrink-0 ${statusStyle[tx.status] ?? "bg-slate-100 text-slate-600"}`}>{tx.status}</span>
                </div>
              ))}
            </div>
          </CardContent>
